...
void tsprintf_set_verbose(bool verbose) { print_message_verbose = verbose; }

bool tsprintf_get_verbose(void) { return print_message_verbose; }

void tsprintf_set_output(FILE* stream) { print_output_stream = stream; }
...
int tsprintf_init(bool use_spinlock, bool verbose, FILE* stream)
{
    ...
    if (pthread_spin_init(&print_message_spinlock, PTHREAD_PROCESS_SHARED) != 0)
        return -1;
#endif
