...
const uint8_t AO40SHORT_INDEX_OF[] = {
    ,
};
...
int8_t ao40short_decode_rs_8(uint8_t* data, int* eras_pos, int no_eras)
{
    ...
    uint8_t lambda[AO40SHORT_NROOTS + 1],
        s[AO40SHORT_NROOTS]; /* Err+Eras Locator poly and syndrome poly */
    ...
    for (j = 1; j < AO40SHORT_NN - AO40SHORT_PAD; j++) {
        for (i = 0; i < AO40SHORT_NROOTS; i++) {
            if (s[i] == 0) {
                ...
                s[i] =
                    data[j] ^
                    AO40SHORT_ALPHA_TO[AO40SHORT_MODNN(
                        AO40SHORT_INDEX_OF[s[i]] + (AO40SHORT_FCR + i) * AO40SHORT_PRIM)];
            }
        }
    }
    ...
    if (no_eras > 0) {
        ...
        lambda[1] = AO40SHORT_ALPHA_TO[AO40SHORT_MODNN(AO40SHORT_PRIM *
                                                       (AO40SHORT_NN - 1 - eras_pos[0]))];
        for (i = 1; i < no_eras; i++) {
        ...
        count = 0;
        for (i = 1, k = AO40SHORT_IPRIM - 1; i <= AO40SHORT_NN;
             i++, k = AO40SHORT_MODNN(k + AO40SHORT_IPRIM)) {
            q = 1;
            for (j = 1; j <= no_eras; j++)
    ...
    while (++r <= AO40SHORT_NROOTS) { /* r is the step number */
        ...
        for (i = 0; i < r; i++) {
            if ((lambda[i] != 0) && (s[r - i - 1] != AO40SHORT_A0)) {
                discr_r ^= AO40SHORT_ALPHA_TO[AO40SHORT_MODNN(
                    AO40SHORT_INDEX_OF[lambda[i]] + s[r - i - 1])];
            }
        }
        ...
        if (discr_r == AO40SHORT_A0) {
            ...
            for (i = 0; i < AO40SHORT_NROOTS; i++) {
                if (b[i] != AO40SHORT_A0)
                    t[i + 1] = lambda[i + 1] ^
                               AO40SHORT_ALPHA_TO[AO40SHORT_MODNN(discr_r + b[i])];
                else
                    t[i + 1] = lambda[i + 1];
            }
            if (2 * el <= r + no_eras - 1) {
                ...
                for (i = 0; i <= AO40SHORT_NROOTS; i++)
                    b[i] = (lambda[i] == 0)
                               ? AO40SHORT_A0
                               : AO40SHORT_MODNN(AO40SHORT_INDEX_OF[lambda[i]] - discr_r +
                                                 AO40SHORT_NN);
            } else {
                /* 2 lines below: B(x) <-- x*B(x) */
                memmove(&b[1], b, AO40SHORT_NROOTS * sizeof(b[0]));
    ...
    for (j = count - 1; j >= 0; j--) {
        ...
        num2 = AO40SHORT_ALPHA_TO[AO40SHORT_MODNN(root[j] * (AO40SHORT_FCR - 1) +
                                                  AO40SHORT_NN)];
        den = 0;
        ...
        if (num1 != 0 && loc[j] >= AO40SHORT_PAD) {
            data[loc[j] - AO40SHORT_PAD] ^= AO40SHORT_ALPHA_TO[AO40SHORT_MODNN(
                AO40SHORT_INDEX_OF[num1] + AO40SHORT_INDEX_OF[num2] + AO40SHORT_NN -
                AO40SHORT_INDEX_OF[den])];
        }
    }
