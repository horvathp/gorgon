...
int8_t ao40_decode_rs_8(uint8_t* data, int* eras_pos, int no_eras)
{
    ...
    uint8_t lambda[AO40_NROOTS + 1],
        s[AO40_NROOTS]; /* Err+Eras Locator poly and syndrome poly */
    ...
    for (j = 1; j < AO40_NN - AO40_PAD; j++) {
        for (i = 0; i < AO40_NROOTS; i++) {
            if (s[i] == 0) {
                ...
                s[i] = data[j] ^ AO40_ALPHA_TO[AO40_MODNN(AO40_INDEX_OF[s[i]] +
                                                          (AO40_FCR + i) * AO40_PRIM)];
            }
        }
    }
    ...
    if (no_eras > 0) {
        ...
        count = 0;
        for (i = 1, k = AO40_IPRIM - 1; i <= AO40_NN;
             i++, k = AO40_MODNN(k + AO40_IPRIM)) {
            q = 1;
            for (j = 1; j <= no_eras; j++)
                if (reg[j] != AO40_A0) {
    ...
    while (++r <= AO40_NROOTS) { /* r is the step number */
        ...
        for (i = 0; i < r; i++) {
            if ((lambda[i] != 0) && (s[r - i - 1] != AO40_A0)) {
                discr_r ^=
                    AO40_ALPHA_TO[AO40_MODNN(AO40_INDEX_OF[lambda[i]] + s[r - i - 1])];
            }
        }
        ...
        if (discr_r == AO40_A0) {
            ...
            if (2 * el <= r + no_eras - 1) {
                ...
                for (i = 0; i <= AO40_NROOTS; i++)
                    b[i] = (lambda[i] == 0)
                               ? AO40_A0
                               : AO40_MODNN(AO40_INDEX_OF[lambda[i]] - discr_r + AO40_NN);
            } else {
                /* 2 lines below: B(x) <-- x*B(x) */
                memmove(&b[1], b, AO40_NROOTS * sizeof(b[0]));
                b[0] = AO40_A0;
    ...
    for (j = count - 1; j >= 0; j--) {
        ...
        if (num1 != 0 && loc[j] >= AO40_PAD) {
            data[loc[j] - AO40_PAD] ^=
                AO40_ALPHA_TO[AO40_MODNN(AO40_INDEX_OF[num1] + AO40_INDEX_OF[num2] +
                                         AO40_NN - AO40_INDEX_OF[den])];
        }
    }
