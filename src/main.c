...
int block_fifo_write(block_fifo_t* fifo, void* block, bool overwrite)
{
    ...
    if ((fifo->unread_cnt >= fifo->fifo_sizem1)) { // fifo is full
        ...
        // ACCESS_UNLOCK;
        // sched_yield();		// try to hand over processing power to the reader side
        // if the fifo is full ACCESS_LOCK;
    ...
    if (fifo->unread_cnt == 1)
        pthread_cond_signal(
            &fifo->fifo_cond); // signaling only if the fifo becomes non-empty
...
bool timespec_expired(struct timespec deadline)
{
    ...
    if (timeout_time.tv_sec < deadline.tv_sec)
        return false;
    if (timeout_time.tv_sec > deadline.tv_sec)
        return true;

    if (timeout_time.tv_nsec < deadline.tv_nsec)
        return false;
    return true;
}
...
void* input_ptfun(void* arguments)
{
    ...
    do {
        ...
        if (input_buffer == NULL)
            global_break("Getline failed\n");
        if (read_len < 0) { // input is gone!
            tsprintf("STDIN reached EOF\nWait for processing...\n");
            while (!block_fifo_empty(&received_raw_packet_fifo))
                us_sleep();
            while (!block_fifo_empty(&received_decoded_packet_fifo))
                us_sleep();
        ...
        if (read_len < 8192) {
            ...
            for (i = 0; i < read_len; i++) {
                if (received_pckt->pckt_len > 1024)
                    break;
                if (i % 2 == 0) { // upper nibble
                    ...
                    received_pckt->pckt[received_pckt->pckt_len] =
                        (hex_to_num(upper_case(input_buffer[i])) << 4);
                } else { // lower nibble
                    ...
                    received_pckt->pckt[received_pckt->pckt_len] +=
                        hex_to_num(upper_case(input_buffer[i]));
                    received_pckt->pckt_len += 1;
                }
            }
            ...
            if (received_pckt->pckt_len > 0) { // may be valid packet
                ...
                for (; i < read_len; i++) { // 'i' continues
                    switch (k) {
                    case 0:
                        k = ((input_buffer[i] == 'R') ? (k + 1) : 0);
                        break;
                    case 1:
                        k = ((input_buffer[i] == 'S') ? (k + 1) : 0);
                        break;
                    case 2:
                        k = ((input_buffer[i] == 'S') ? (k + 1) : 0);
                        break;
                    case 3:
                        k = ((input_buffer[i] == 'I') ? (k + 1) : 0);
                        break;
                    case 4:
                        k = ((input_buffer[i] == ' ') ? (k + 1) : 0);
                        break;
                    default:
                        k = 0;
                        break;
                ...
                if (rssi_valid &&
                    (received_pckt->pckt_len <= 1024)) { // hand over the received packet
...
void* rx_pckt_proc_ptfun(void* arguments)
{
    ...
    do {
        if (block_fifo_read(&received_raw_packet_fifo, received_pckt) ==
            0) {                                  // blocking read
            if (received_pckt->pckt_len >= 650) { // only DATA
                for (i = 0; i < 4; i++)
                    dtarg[i].decoded_pckt->rssi = received_pckt->rssi;
                ...
                for (i = 3; i >= 0; i--) {
                    ...
                    if (dtarg[i].decoded_pckt->pckt_len > 0) {
                        if (packet_check(dtarg[i].decoded_pckt->pckt,
                                         dtarg[i].decoded_pckt->pckt_len) == 0)
                            block_fifo_write(&received_decoded_packet_fifo,
                                             dtarg[i].decoded_pckt,
                                             false); // packet_check is low cost
                    }
                }
            }
        }
...
void* rx_pckt_data_ptfun(void* arguments)
{
    ...
    do {
        if (block_fifo_read(&received_decoded_packet_fifo, received_pckt) ==
            0) { // blocking read

            // check expected sizes
            switch (received_pckt->pckt[0]) {
            case DOWNLINK_PCKT_TYPE_TELEMETRY_1:
                if (received_pckt->pckt_len != 128)
                    continue;
                break;
            ...
            if (add_check_duplicate_signature(received_pckt->pckt,
                                              received_pckt->pckt_len))
                continue; // do not process packets with duplicate signature

            tsprintf(
                "Received %db long packet with %d RSSI;\tAUTHENTICATION: not available\n",
                received_pckt->pckt_len,
                received_pckt->rssi);
...
void pckt_proc_DOWNLINK_PCKT_TYPE_TELEMETRY_1(pckt_t* pckt)
{
    ...
    pcktprint_open("ct1");
    int pphex;
    for (pphex = 0; pphex < pckt->pckt_len; pphex++)
        pcktonlyfileprintf("%02hhX", pckt->pckt[pphex]);
    pcktonlyfileprintf("\n");
    ...
    for (i = 0; i < 6; i++) {
        ...
        switch ((u8 >> 3) & 0x03) {
        case 0:
            pcktprintf("\t\tAntenna status: closed\n");
            break;
        case 1:
            pcktprintf("\t\tAntenna status: open\n");
            break;
        case 2:
            pcktprintf("\t\tAntenna status: not monitored\n");
            break;
        case 3:
            pcktprintf("\t\tAntenna status: INVALID!\n");
            break;
        }
    }
...
void pckt_proc_DOWNLINK_PCKT_TYPE_SPECTRUM_RESULT(pckt_t* pckt)
{
    ...
    pcktprintf("\tMeasured %hu samples between %.6fMHz ... %.6fMHz with %.3fkHz stepsize "
               "and %hhu RBW\n",
               spectrum_len,
               startfreq / 1000000.0,
               stopfreq / 1000000.0,
    ...
    if (asap_spectrum == NULL) {
        if (pckt_index == 0) {
            ...
            if (asap_spectrum->data == NULL)
                goto spectrum_error;
            for (i = 0; i < (pckt_cnt * 224); i++)
                asap_spectrum->data[i] = 0;
        }
    } else {
        if (pckt_index == 0) { // ?? -> discard
            ...
            if (asap_spectrum->timestamp != tmp_time)
                goto spectrum_error;
            if (asap_spectrum->startfreq != freqs[0])
                goto spectrum_error;
            if (asap_spectrum->stepfreq != freqs[1])
                goto spectrum_error;
            if (asap_spectrum->rbw != rbw)
                goto spectrum_error;
            if (asap_spectrum->pckt_cnt != pckt_cnt)
                goto spectrum_error;
            if (asap_spectrum->measid != measid)
                goto spectrum_error;
        }
    }
    ...
    if (asap_spectrum->spectrum_len > (asap_spectrum->pckt_cnt * 224))
        goto spectrum_error;
    if (spectrum_len > 224)
        goto spectrum_error;
    ...
    if (pckt_index == (pckt_cnt - 1)) { // done!
        ...
        if (fsa != NULL) {
            ...
            tmp_str_len = sprintf(tmp_str,
                                  "ID;%hu\nRBW;%hhu\nFrequency;Amplitude\n",
                                  asap_spectrum->measid,
                                  asap_spectrum->rbw);
            if (tmp_str_len < 0)
                goto spectrum_error;
            fwrite(tmp_str, 1, tmp_str_len, fsa);
            ...
            for (i = 0; i < asap_spectrum->spectrum_len; i++) {
                tmp_str_len =
                    sprintf(tmp_str,
                            "%f;%hhu\n",
                            asap_spectrum->startfreq + i * asap_spectrum->stepfreq,
                            asap_spectrum->data[i]);
                if (tmp_str_len < 0)
                    goto spectrum_error;
                fwrite(tmp_str, 1, tmp_str_len, fsa);
            }
...
void pckt_proc_DOWNLINK_PCKT_TYPE_FILE_INFO(pckt_t* pckt)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        pcktprintf("\tFile ID=%hhu, type=%hhu, name=\"%s\" size=%u, starting at page "
                   "%hu. Timestamp: %s\n",
                   file_id,
                   file_type,
                   file_name,
...
void pckt_proc_DOWNLINK_PCKT_TYPE_FILE_FRAGMENT(pckt_t* pckt)
{
    ...
    pcktprintf("\tDownloading %hu/%hu fragment of the file \"%s\" with type=%hhu, "
               "size=%u at page %hu. Timestamp: %s\n",
               pckt_index,
               pckt_cnt,
               file_name,
               file_type,
    ...
    if (strlen(file_name) == 0) {
        sprintf(
            file_shadow_name, "partial/.noname_%02hX_%s.dat", page_addr / 128, timestr);
        sprintf(
            file_content_name, "partial/noname_%02hX_%s.dat", page_addr / 128, timestr);
    } else {
        sprintf(file_shadow_name,
                "partial/.%s_%02hX_%s.dat",
                file_name,
                page_addr / 128,
                timestr);
        sprintf(file_content_name,
                "partial/%s_%02hX_%s.dat",
                file_name,
                page_addr / 128,
                timestr);
    ...
    if ((file_shadow == NULL) || (file_content == NULL))
        goto fileop_error; // at this point both file should be opened
    ...
    if (pckt_index < (pckt_cnt - 1)) { // not the last block
        ...
        if ((block_size < 0) || (block_size > 217))
            goto fileop_error;
    }
    ...
    if (i == pckt_cnt) { // complete file
        ...
        memcpy(file_shadow_name,
               "download/",
               9); // reuse string: replace "partial/." to "download/"
...
void pckt_proc_DOWNLINK_PCKT_TYPE_SMOGP_TELEMETRY_1(pckt_t* pckt)
{
    ...
    if (u8 == '0')
        ...
        pcktprintf("\tOBC ID: invalid!\n");

    memcpy(&u8, &buf[p], 1);
    p += 1;
    if (u8 == 'E')
        pcktprintf("\tOscillator: external\n");
    else if (u8 == 'I')
        pcktprintf("\tOscillator: internal\n");
    else
        pcktprintf("\tOscillator: invalid!\n");
    ...
    p += 1;
    if (u8 & 0x80)
        pcktprintf("\tSPI-MUX status: functional\n");
    else
        pcktprintf("\tSPI-MUX status: not functional\n");
    ...
    p += 1;
    if (u8 == 0)
        pcktprintf("\tSPI/FLASH status: not functional\n");
    else
        pcktprintf("\tSPI/FLASH status: functional, startcount == %hhu\n", u8);

    memcpy(&u8, &buf[p], 1);
    p += 1;
    if (u8 == 0)
        pcktprintf("\tSPI/MSEN status: not functional\n");
    else
        pcktprintf("\tSPI/MSEN status: functional, startcount == %hhu\n", u8);

    memcpy(&u8, &buf[p], 1);
    p += 1;
    if (u8 == 0)
        pcktprintf("\tSPI/RTCC status: not functional\n");
    else
        pcktprintf("\tSPI/RTCC status: functional, startcount == %hhu\n", u8);
    ...
    switch (u8) {
    case 0:
        tsprintf("\x1b[1m\x1b[32m"); // bold green
        pcktprintf("\tEnergy Management Mode: normal (morse period: %hhu; radio cycle: "
                   "%.1fs; sleep: 0)",
                   emm_mp,
                   emm_rc / 1e6);
        break;
    case 1:
        tsprintf("\x1b[1m\x1b[32m"); // bold green
        pcktprintf("\tEnergy Management Mode: normal, reduced (morse period: %hhu; radio "
                   "cycle: %.1fs; sleep: %.1fs)",
                   emm_mp,
                   emm_rc / 1e6,
                   emm_sleep / 1e6);
        break;
    case 2:
        tsprintf("\x1b[1m\x1b[33m"); // bold yellow
        pcktprintf("\tEnergy Management Mode: energy saving (NO morse; radio cycle: "
                   "%.1fs; sleep: %.1fs",
                   emm_rc / 1e6,
                   emm_sleep / 1e6);
        break;
    case 3:
        tsprintf("\x1b[1m\x1b[101m"); // red background
        pcktprintf("\tEnergy Management Mode: EMERGENCY (NO morse; radio cycle: %.1fs; "
                   "sleep: %.1fs",
                   emm_rc / 1e6,
                   emm_sleep / 1e6);
        break;
...
void pckt_proc_DOWNLINK_PCKT_TYPE_SMOGP_TELEMETRY_2(pckt_t* pckt)
{
    ...
    if (u8 == 'V') {
        ...
        if (u8 == 'Y') {
            ...
            pcktprintf("\tMSEN Magneto MIN-MAX: [% 5.0f ... % 5.0f], [% 5.0f ... % "
                       "5.0f], [% 5.0f ... % 5.0f]\n",
                       1.0 * vector[0],
                       1.0 * vector[3],
                       1.0 * vector[1],
...
int main(int argc, char* argv[])
{
    ...
    if (block_fifo_init(&received_raw_packet_fifo, sizeof(pckt_t), 256) < 0)
        global_break("FIFO init failed\n");
    if (block_fifo_init(&received_decoded_packet_fifo, sizeof(pckt_t), 256) < 0)
        global_break("FIFO init failed\n");
